...
class MappedStats(APIView):
    ...
    def get(self, request):
        ...
        # tot mapped uniprot entries
        uniprot_mapped_count = Mapping.objects.values('uniprot').distinct().count()

        # tot non mapped Swiss-Prot entries, NOTE: NO WAY TO GET IT AT THE MOMENT
        uniprot_not_mapped_sp_count = None

        all_entry_types = {}
        for entry in CvEntryType.objects.all():
            all_entry_types[entry.id] = entry.description

        sp_entry_type_ids = []
        for (k, v) in all_entry_types.items():
            if v.lower().startswith('swiss'):
                sp_entry_type_ids.append(k)
        ...
        # tot mapped Swiss-Prot entries, NOTE: NOT REQUESTED BUT HERE FOR COMPLETENESS
        uniprot_mapped_sp_count = 0
        ...
        # tot mapped Ensembl genes
        gene_mapped_count = len(gene_mapped_ids)
        gene_not_mapped_sp_count = len(gene_ids.difference(gene_mapped_ids))

        # tot mapped Ensembl transcripts
        transcript_mapped_count = Mapping.objects.values('transcript').distinct().count()
        ...
        #
        status_counts = []
        status_totals = Mapping.objects.values(
            'status'
        ).annotate(
            total=Count('status')
        )
        ...
        for status_count in status_totals:
            status_counts.append({
                'status': Mapping.status_type(status_count['status']),
